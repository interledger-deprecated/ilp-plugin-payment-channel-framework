...
describe('Conditional Transfers', () => {
  beforeEach(function * () {
    ...
    this.incomingTransferObj = Object.assign({}, this.transferObj, {
      from: peerAddress,
      to: this.plugin.getAccount()
    })
    this.incomingTransfer = btpPacket.serializePrepare(
      Object.assign({}, this.incomingTransferObj, {transferId: this.incomingTransferObj.id}),
      requestId + 2,
      protocolMapToProtocolData(this.incomingTransferObj.custom)
    )
  ...
  describe('sendTransfer (conditional)', () => {
    it('allows an outgoing transfer to be fulfilled', function * () {
      ...
      yield this.plugin.sendTransfer(this.transferObj)
      yield sent
    ...
    it('fulfills an incoming transfer', function * () {
      ...
      yield this.plugin.fulfillCondition(this.transferObj.id, this.fulfillment)
      yield fulfilled
    ...
    it('cancels an incoming transfer for too much money', function * () {
      this.incomingTransferObj.amount = 100
      this.incomingTransferObj.transferId = this.incomingTransferObj.id
      const transfer = btpPacket.serializePrepare(
        this.incomingTransferObj,
        12345,
        protocolMapToProtocolData(this.incomingTransferObj.custom)
      )
    ...
    it('doesn\'t fulfill a transfer with invalid fulfillment', function * () {
      ...
      yield this.plugin.sendTransfer(this.transferObj)
      yield expect(this.plugin.fulfillCondition(this.transferObj.id, 'Garbage'))
    ...
    it('doesn\'t fulfill an outgoing transfer', function * () {
      ...
      yield this.plugin.sendTransfer(this.transferObj)
      yield expect(this.plugin.fulfillCondition(this.transferObj.id, this.fulfillment))
        .to.eventually.be.rejected
    })

    it('should not send a transfer with condition and no expiry', function () {
      this.transferObj.executionCondition = undefined
      return expect(this.plugin.sendTransfer(this.transferObj)).to.eventually.be.rejected
    })

    it('should not send a transfer with expiry and no condition', function () {
      this.transferObj.expiresAt = undefined
      return expect(this.plugin.sendTransfer(this.transferObj)).to.eventually.be.rejected
    })
  ...
  describe('expireTransfer', () => {
    it('expires a transfer', function * () {
      this.mockSocket
        ...
        .reply(btpPacket.TYPE_REJECT, ({requestId, data}) => {
          assert.equal(data.transferId, this.incomingTransferObj.transferId)
      ...
      this.incomingTransferObj.expiresAt = (new Date(Date.now() + 10)).toISOString()
      this.incomingTransferObj.transferId = this.incomingTransferObj.id
    ...
    it('expires an outgoing transfer', function * () {
      this.transferObj.expiresAt = (new Date()).toISOString()
    ...
    it('doesn\'t expire an executed transfer', function * () {
      ...
      yield this.plugin._rpc.handleMessage(this.mockSocketIndex, this.btpFulfillment)
      yield fulfilled
      yield this.plugin._expireTransfer(this.transferObj.id)
  ...
  describe('rejectIncomingTransfer', () => {
    it('rejects an incoming transfer', function * () {
      ...
      yield this.plugin._rpc.handleMessage(this.mockSocketIndex, this.incomingTransfer)
      yield this.plugin.rejectIncomingTransfer(this.transferObj.id, expectedRejectionReason)
      yield rejected
    ...
    it('should not reject an outgoing transfer', function * () {
      ...
      yield this.plugin.sendTransfer(this.transferObj)
      yield expect(this.plugin.rejectIncomingTransfer(this.transferObj.id, 'reason'))
        .to.eventually.be.rejected
    })
